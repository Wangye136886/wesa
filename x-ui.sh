...
install() {
    bash <(curl -Ls https://gitlab.com/ye857257/wesa/master/install/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://gitlab.com/ye857257/wesa/master/install/x-ui/master/install/wesa/master/install.sh)
    if [[ $? == 0 ]]; then
        LOGI "更新完成，已自动重启面板 "
        exit 0
    fi
...
install_bbr() {
    ...
    bash <(curl -L -s https://gitlab.com/teddysun/across/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/Wangye136886/wesa/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
